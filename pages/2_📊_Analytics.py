...
cards_metric_df = df.groupby(["Category"])['Actual_Price'].mean().reset_index()

fruits = cards_metric_df.loc[cards_metric_df["Category"] == "Fruit"]
fruits_val = fruits.Actual_Price.values[0]
fruits_val = round(fruits_val, 2)


herbs = cards_metric_df.loc[cards_metric_df["Category"] == "Herbs and Spices"]
herbs_val = herbs.Actual_Price.values[0]
herbs_val = round(herbs_val, 2)

legumes = cards_metric_df.loc[cards_metric_df["Category"] == "Legumes"]
legumes_val = legumes.Actual_Price.values[0]
legumes_val = round(legumes_val, 2)

roots = cards_metric_df.loc[cards_metric_df["Category"] == "Root Crops"]
roots_val = roots.Actual_Price.values[0]
roots_val = round(roots_val, 2)

vegetables = cards_metric_df.loc[cards_metric_df["Category"] == "Vegetable"]
vegetables_val = vegetables.Actual_Price.values[0]
vegetables_val = round(vegetables_val, 2)
...
df_hp = df.groupby(["month", "month1"])['Actual_Price'].mean().reset_index()
...
with row2_col1:
    st.plotly_chart(fig_chloro, use_container_width=True, theme="streamlit")
...
df_avail0 = df.groupby(["year", "supply"])["Actual_Price"].mean().reset_index()
